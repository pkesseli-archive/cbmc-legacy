    ...
    const goto_programt::targett &pos)
{
  ...
  const symbol_exprt smb(as_symbol(prog.get_st(), get_affected_variable(*pos)));
    ...
    const invariant_programt::invariant_loopt &loop, const size_t loop_id)
{
  ...
  const symbol_tablet &st=prog.get_st();
...
{
  const safety_programt &safety=prog.safety;
...
{
  const symbol_tablet &st=prog.get_st();
  goto_functionst &gf=prog.get_gf();
